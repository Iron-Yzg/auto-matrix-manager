...
async function main() {
    ...
    try {
        ...
        // 获取 CDP 会话以使用 Network 事件（必须在页面导航前附加）
        ...
        cdpSession.on('Network.requestWillBeSent', (event) => {
            ...
            if (url.includes('/web/api/media/user/info')) {
                ...
                if (userInfoHeaders[url].cookie) {
                    console.log('[CDP] userInfo Cookie preview: ' + userInfoHeaders[url].cookie.substring(0, 100));
                }
            }
            ...
            if (url.includes('/account/api/v1/user/account/info')) {
                ...
                if (accountInfoHeaders[url].cookie) {
                    console.log('[CDP] accountInfo Cookie preview: ' + accountInfoHeaders[url].cookie.substring(0, 100));
                }
            }
        });
        ...
        page.on('response', async (response) => {
            ...
            if (url.includes('/web/api/media/user/info')) {
                ...
                if (Object.keys(userInfoHeaders).length === 0) {
                    ...
                    console.log('[Response] Fallback: userInfo Cookie length: ' + (userInfoHeaders[url].cookie || '').length);
                }
            ...
            if (url.includes('/account/api/v1/user/account/info')) {
                ...
                if (Object.keys(accountInfoHeaders).length === 0) {
                    ...
                    console.log('[Response] Fallback: accountInfo Cookie length: ' + (accountInfoHeaders[url].cookie || '').length);
                }
        ...
        // 获取浏览器 cookies（用于 result.cookie）
        const cookies = await page.context().cookies();
        console.log('[Info] Browser cookies captured:', cookies.length);
        if (cookies.length > 0) {
            console.log('[Info] Browser cookie names:', cookies.map(c => c.name).join(', '));
        }

        // 等待页面完全加载完成后再获取 localStorage（security-sdk 的 key 可能在页面加载后才设置）
        console.log('[Info] Waiting for localStorage to be ready...');
        await page.waitForLoadState('networkidle');
        await page.waitForTimeout(2000);  // 额外等待确保 security-sdk 初始化完成

        // 获取 localStorage（与 Python 逻辑完全一致）
        ...
        console.log('[Info] localStorage items count:', Object.keys(localStorageScript).length);
        console.log('[Info] localStorage keys:', Object.keys(localStorageScript).join(', '));
        if (localStorageScript['security-sdk/s_sdk_cert_key']) {
            console.log('[Info] s_sdk_cert_key length:', localStorageScript['security-sdk/s_sdk_cert_key'].length);
        }
